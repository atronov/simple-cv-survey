    ...
    <RecoilRoot>
      <div className={styles.app}>
        ...
        {questions.sections.map((sectionData, i) => (
          <Section sectionIndex={i} data={sectionData} />
        ))}
      </div>
    </RecoilRoot>
  );
...
const Completion = () => {
  ...
  return (
    ...
    >
      {texts.totalScore} {totalScore}/{maxScore}. {texts.passingScore}{" "}
      {passingScore}.{" "}
      {accepted ? (
        <>
...
const Header = () => {
  const { answeredCount, totalCount, accepted, completed } =
    useRecoilValue(progressState);
...
const Point = (props: PointProps) => {
  ...
  const handlePointChange = useCallback(
    (value: number) =>
      setPoints((oldPoints) =>
        setOnePontScore(oldPoints, sectionInd, questionInd, value)
      ),
  ...
  return (
    <Space>
      ...
      />
      <Rate
        value={Math.max(point, 0)}
        count={maxPointScore}
        onChange={handleSetPositiveRate}
      />
    </Space>
  );
};
...
${texts.totalScore} ${totalScore}/${maxScore}. ${
    texts.passingScore
  } ${passingScore}. ${accepted ? texts.hasPassed : texts.hasFailed}.

${texts.details}:`;
  const flatQuestions = questions.sections.flatMap((section) =>
